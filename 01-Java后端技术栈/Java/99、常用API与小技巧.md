JDK 标准库常用 API 速查：**能抄的写法 + 踩坑**。第三方库（Hutool、Commons、Jackson）见 [工具类/2、日期时间与JSON工具](../工具类/2、日期时间与JSON工具.md)。
...
| JSON | Spring Boot 配 `Jackson JavaTimeModule`，见 [工具类/2](../工具类/2、日期时间与JSON工具.md) |
...
常用 MD5/SHA 也可直接用 Commons Codec / Hutool，见 [工具类/1](../工具类/1、Java与第三方常用工具库.md)。
