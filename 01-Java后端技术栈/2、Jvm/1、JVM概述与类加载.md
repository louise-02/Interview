...
![image-20260616142852336](./pictures/image-20260616142852336.png)
...
| 顺序 | 内容 | 说明 |
| ---- | ---- | ---- |
| 1 | **静态代码块** | 类**首次加载**时执行，只一次；对应 `<clinit>()` |
| 2 | **实例代码块**（`{}`） | 每次 `new` 执行，**先于**构造方法体；编译进 `<init>()` |
| 3 | **构造方法** | 每次 `new` 执行 |
| — | **普通代码块** | 方法/分支里的 `{}`，**不在** `new` 链路里，执行到才运行 |
...
记忆：**先静后实，先父后子**。父类 `<clinit>()` 先于子类；`new` 子类时父类 `<init>()` 先于子类。

```java
class Parent {
    static { System.out.println("父 static"); }
    { System.out.println("父 实例块"); }
    Parent() { System.out.println("父 构造"); }
}
class Child extends Parent {
    static { System.out.println("子 static"); }
    { System.out.println("子 实例块"); }
    Child() { System.out.println("子 构造"); }
}
// new Child()：
// 父 static → 子 static → 父 实例块 → 父 构造 → 子 实例块 → 子 构造
```

**3. 两个 ClassLoader 加载同一个 .class 文件，是同一个 Class 吗？**
