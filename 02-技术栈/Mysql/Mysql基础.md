...
系统读取数据的时候**按照磁盘块来读取的**,在同一个磁盘块上的数据可以一起读出来.(文件系统不同磁盘块的大小不同,但是相同的文件系统,磁盘块的大小是固定的)。

mysql中读取数据的时候是**按照页(page默认16kb)**来读的,16kb包含的磁盘块数量是固定的(因为相同的文件系统上磁盘块的大小是固定的.所以可以认为一页上包含的磁盘块数量也是固定的)。

InnoDB的一个页的大小是16KB,所以叶子节点和非叶子节点都是16KB, 并且B+Tree的根节点是保存在内存中的,子节点才是存储在磁盘上。

![image.png](pictures/ed746f7baf004f1e92a18649e3000bd0.png)
...
图中是一张字段为id、name、db的表，创建name为辅助索引，辅助索引中存储该字段与主键索引，通过name来检索数据的时候会先在辅助索引中找到主键，再通过主键来检索到整行。执行一次查询就是一次IO,如果辅助索引搜索深度为3，主键索引深度为2，那检索数据就要进行3+2次逻辑IO。
...
![image-20260615163136485](./pictures/image-20260615163136485.png)
...
-- select_type 查询类型
simple ——简单的select查询，查询中不包含子查询或者UNION 例如: select name from student where id= 100
...
- **log_output**：表示将日志存入文件，默认值是'FILE'，'TABLE'表示将日志存入数据库，这样日志信息就会被写入到 mysql.slow_log 表中。也支持'FILE,TABLE'
...
6. 只获取自己需要的列，不要使用select  *　筛选出需要使用的列
7. 尽可能避免复杂的join和子查询
...
- 使用覆盖索引
- 复合索引遵循最左原则
- 确保join的数据类型一致，不然mysql内部会隐式转换数据类型
- 如果join的类型为字符串，需要看字符集是否一致
...
**INT(1)**，**INT(10)**存储占用都为4个字节（32位），可以存储有符号-2147483648到2147483647，无符号0到4294967295的整数。而长度指定了整数的**显示宽度**，表示这个整数在显示时最多显示的位数。但是，这个显示宽度仅仅是用于显示的控制，并不限制存储的范围。

想要在存储整数时进行填充，以保持显示宽度的长度，需要用 **ZEROFILL** 属性与整数类型一起使用，以指示在存储整数时填充零字符。

例如：定义ZEROFILL 的列，并存储整数值为 123，MySQL会将其存储为 0123。这样，在查询结果中以字符串形式显示时，整数值将占用4个字符的宽度，并在左侧填充零字符。

需要注意的是，ZEROFILL 属性只对显示宽度大于实际整数位数的情况有效。如果整数值的位数超过了显示宽度，ZEROFILL 属性不会截断或调整整数值的宽度。

# !=会不会查询到null值

不会，null值需要单独用 `IS NULL` 进行筛选。